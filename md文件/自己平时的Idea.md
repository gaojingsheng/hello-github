...
刚刚我想到一个小样本学习的方向，我们是不是可以定义一个方法，无脑对样本进行扩增，实现“专门”学习，样本扩增后，网络就能够进行拟合而不至于太“过拟合”。加上这个了之后，我们就能看到几次马了之后，以后看到其他的马就能自信的说它是马。它的名字应该可以叫**Single-Sample learning!!!**

我觉得我们得通过我们之前的东西知道如何去发挥自己的想象力，预训练好一个模型对数据进行想象增广。

怎么实现呢？我觉得可以用GAN来做。

## 2020.7.27

迁移学习可能和我上次的idea有异曲同工之妙，特别是特征迁移，我的想法是将特征加在其他的东西上进行扩增。

## 2020.7.30

忽然想到量子力学本身的波粒二象性会不会有助于深度学习拟合。（刚刚仔细一想发现我在扯淡，波粒二象性是描述微观粒子的状态是一个波函数可以发生干涉衍射等性质，波函数描述的是粒子的概率密度函数，如果想用波函数来描述神经元或者图像场的话，那尼玛就是扯淡）

## 2020.8.7

今天想的是梯度下降的过程，

1、最传统的方法就是沿着导数的反方向进行下降，然后根据学习率不同进行收敛，学习率太低容易陷入局部最小值，学习率太高容易不收敛。

2、然后梯度下降加入动量过程，记住原来的下降方向再下降，有助于摆脱局部最小值，所以我们可以设置更小的学习率进行更加平滑的下降。

3、AdaGrad方法，累计历史上出现过的梯度（平方）,因为一开始的梯度比较大，学习率容易过早过量的减少。

4、RMSProp加入了平方梯度的衰减项，只能记录最近一段时间的梯度，总之都是通过记录之前的梯度来控制学习率。

5、Adam算法 用一阶记录梯度，用二阶控制学习率，怎么说呢，需要调参。

6、牛顿方法，

**刚刚想了一个idea**：就是我们训练的时候都用选用loss在选择不同optimizer(梯度下降方法)进行训练，一开始的output只和loss有关系，那我们可以不可以直接将output对前传播，先假想没有输入？但是如果输入等于0，那么一开始的参数就是加啰。我得想想，如何有效的利用output对前传播可能有效。

## 2020.8.10

刚想到一个发paper的故事，我们平时在实验自己的trick的时候都是去对比训练完整的数据集，这样的问题就是耗时太久了，没法迅速知道自己的trick是否真的很有效。我们是不是可以直接创建一套方法去迅速判断trick的有效性？
